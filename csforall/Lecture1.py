...
class CS5Frame(Frame):
    ...
    def showList(self, L = None, newColors = {}):
        ...
        if not L:
            # we'll just make up 5 random colors...
            self.canv.show1d( [ [random.randrange(0,256) for i in range(3)] for i in range(5) ], newColors ) 
        else:
...
#
# These next three functions are for Lab 3
#
...
def ib(a,b):
    if 0 <= a <= 9 and 0 <= b <= 9: return True
    return False

def updateLdata(c,r,sz,clr,dir):
    global Ldata
    COLOR = 0
    if clr == 'lime':
        COLOR = [0,255,0]
    ...
    if dir == 'NE':
        ...
        cr = r-1
        for i in range(sz):
            if ib(cr,cc+i): Ldata[cr][cc+i] = COLOR
            if ib(cr-i,cc): Ldata[cr-i][cc] = COLOR
...
def L(col,row,size,color,dir):
    """ displays a 2d array of colored squares """
    global Ldata
    updateLdata(col,row,size,color,dir)
    w = _getwin()
    show( [0] ) # clears things
    w.canv.add2d(Ldata)
    w.update()
...
# import Lecture1 ; reload(Lecture1) ; from Lecture1 import *
